...
#----------------------------------------------------------
  ...
  jal   mainprocess0              # go to program
  halt
...
generaterandom:
  ...
  addi  $t7, $t7, 1
  bne   $t7, $t6, generaterandom   # branch as long as counter is not equal to 256         
...
# Second Processor
#----------------------------------------------------------
  org   0x1000               # second processor
  ori   $sp, $zero, 0x7ffc  # stack for second processor
  jal   mainprogram1              # go to program
  halt
...
mainprogram1:
  ...
  #initialize results
  ori   $s0, $zero, 0xFFFFFFFF  # Min val
  ori   $s1, $zero, 0x00000000  # Max val
  ori   $s2, $zero, 0           # Sum of rando vals

popnextvalue:
  # check that there are stack values
  jal   stackcheck

  # obtain locks and pop values
  ori   $a0, $zero, l1      # Move lock to argument reg
  jal   lock                # Acquire lock
  jal   popVal              # Pop top val from stack
  or    $t7, $zero, $v0     # Move popped value to t7
  ori   $a0, $zero, l1      # Move lock to argument register
  jal   unlock              # Release lock
  ...
  # Compute the summation and store in S2
  andi $t7, $t7, 0x0000FFFF
  add $s2, $s2, $t7

  
  # Increment the counter and branch
  addi  $t6, $t6, 1
  bne   $t6, $t5, popnextvalue
...
#---------------------------------------------------------
# Lock and Unlock from example.asm
#---------------------------------------------------------
